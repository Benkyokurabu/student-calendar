...
def is_face_to_face_cell(text: str) -> bool:
    """
    セル内に「対面」を含む授業を対面扱いにする。
    パターン1: "３Ｓ国\\n対面"（2行目に対面）
    パターン2: "２Ｓ国　対面"（同一行に全角/半角スペース区切りで対面）
    """
    if not text:
        return False
    return "対面" in str(text)


def _remove_face_suffix(text: str) -> str:
    """セルテキストから「対面」とその前の空白を除去して本体部分だけ返す"""
    s = str(text or "")
    # 改行で分割した場合の2行目「対面」
    s = re.sub(r"[\r\n]+\s*対面\s*", "", s)
    # 同一行のスペース+対面
...
def normalize_first_line_for_display(text: str) -> str:
    ...
    例: "３Ｓ理" -> "3S理", "２Ｓ国　対面" -> "2S国"
    """
    s = _remove_face_suffix(text)
    lines = s.replace("\r\n", "\n").replace("\r", "\n").split("\n")
...
def convert_events_to_schedule_json(
    ...
    for e in events:
        ...
        special = bool(getattr(e, "special", False))
...
# ===== 特殊授業（英検対策／補講など）の抽出 =====
# 通常授業（学年+科目）として認識されないが、スケジュール表に書かれている
# 「英検対策①」「数学補講③」などのセルを別途拾う。
SPECIAL_KEYWORDS = ("補講", "対策", "英検", "漢検")
...
def collect_special_events_from_sheet(
    ...
    """学年+科目のパターンに当てはまらない特殊授業を抽出する。
    先生は背景色から判定し、teacher フィールドに埋め込む。
    """
    ...
    legend = legacy.detect_teacher_legend(sh)
    room_row = legacy.detect_classroom_row(sh)
    C = column_index_from_string("C")
    AD = column_index_from_string("AD")

    out: List[Dict[str, Any]] = []
    seen = set()

    for r in range(2, 55):
        day, w = legacy.parse_day_week_from_row(sh, r)
        for c in range(C, AD + 1):
            txt = legacy.merged_value_text(sh, r, c)
            if not txt:
                continue
            # 通常授業として認識されるものは対象外
            if legacy.parse_class_token(txt):
                continue
            if not _is_special_text(txt):
                continue

            if day is None or w is None:
                day, w = legacy.parse_day_week_from_row(sh, r, allow_prev_row=True)
            if not day:
                continue
            date_str = ymd(year, month, day)
            if not date_str:
                continue

            # 1行目を原文ラベルとして使う（教室記号①〜⑨を含む）
            lines = [
                ln.strip()
                for ln in str(txt).replace("\r\n", "\n").replace("\r", "\n").split("\n")
                if ln.strip()
            ]
            ...
            group_key = f"{campus_code}_special_{display_title}"
            key = (date_str, (time or "").strip(), campus_code, group_key, room)
            if key in seen:
                continue
            seen.add(key)
...
def export_month_schedule_json(schedule_arg: Optional[str] = None) -> Path:
    ...
    for campus_label, _sheetname, sh in targets:
        ...
        special_rows = collect_special_events_from_sheet(sh, campus_label, year, month)
        all_rows.extend(special_rows)

    all_rows.sort(key=lambda x: (x["date"], x.get("time", ""), x.get("label", "")))
